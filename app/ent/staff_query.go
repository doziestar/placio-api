...
func (sq *StaffQuery) QueryPlace() *PlaceQuery {
	query := (&PlaceClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *StaffQuery) QueryPermissions() *PermissionQuery {
...
func (sq *StaffQuery) QueryBusiness() *BusinessQuery {
	query := (&BusinessClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *StaffQuery) Clone() *StaffQuery {
	...
	return &StaffQuery{
		...
		withPlace:       sq.withPlace.Clone(),
		withPermissions: sq.withPermissions.Clone(),
		withBusiness:    sq.withBusiness.Clone(),
...
func (sq *StaffQuery) WithPlace(opts ...func(*PlaceQuery)) *StaffQuery {
	query := (&PlaceClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *StaffQuery) WithPermissions(opts ...func(*PermissionQuery)) *StaffQuery {
	query := (&PermissionClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withPermissions = query
	return sq
}
...
func (sq *StaffQuery) WithBusiness(opts ...func(*BusinessQuery)) *StaffQuery {
	query := (&BusinessClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withBusiness = query
	return sq
}
...
func (sq *StaffQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Staff, error) {
	...
	if query := sq.withPlace; query != nil {
		if err := sq.loadPlace(ctx, query, nodes,
			func(n *Staff) { n.Edges.Place = []*Place{} },
			func(n *Staff, e *Place) { n.Edges.Place = append(n.Edges.Place, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withPermissions; query != nil {
		if err := sq.loadPermissions(ctx, query, nodes,
			func(n *Staff) { n.Edges.Permissions = []*Permission{} },
			func(n *Staff, e *Permission) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withBusiness; query != nil {
		if err := sq.loadBusiness(ctx, query, nodes,
			func(n *Staff) { n.Edges.Business = []*Business{} },
			func(n *Staff, e *Business) { n.Edges.Business = append(n.Edges.Business, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StaffQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Staff, init func(*Staff), assign func(*Staff, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_staffs == nil {
			continue
		}
		fk := *nodes[i].user_staffs
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_staffs" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *StaffQuery) loadPlace(ctx context.Context, query *PlaceQuery, nodes []*Staff, init func(*Staff), assign func(*Staff, *Place)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Staff)
	nids := make(map[string]map[*Staff]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(staff.PlaceTable)
		s.Join(joinT).On(s.C(place.FieldID), joinT.C(staff.PlacePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(staff.PlacePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(staff.PlacePrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Staff]struct{}{byID[outValue]: {}}
...
func (sq *StaffQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Staff, init func(*Staff), assign func(*Staff, *Permission)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Staff)
	nids := make(map[string]map[*Staff]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(staff.PermissionsTable)
		s.Join(joinT).On(s.C(permission.FieldID), joinT.C(staff.PermissionsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(staff.PermissionsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(staff.PermissionsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (sq *StaffQuery) loadBusiness(ctx context.Context, query *BusinessQuery, nodes []*Staff, init func(*Staff), assign func(*Staff, *Business)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Staff)
	nids := make(map[string]map[*Staff]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(staff.BusinessTable)
		s.Join(joinT).On(s.C(business.FieldID), joinT.C(staff.BusinessPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(staff.BusinessPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(staff.BusinessPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
