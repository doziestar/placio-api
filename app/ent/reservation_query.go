...
func (rq *ReservationQuery) QueryPlace() *PlaceQuery {
...
func (rq *ReservationQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (rq *ReservationQuery) WithPlace(opts ...func(*PlaceQuery)) *ReservationQuery {
	query := (&PlaceClient{config: rq.config}).Query()
...
func (rq *ReservationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Reservation, error) {
	...
	if query := rq.withUser; query != nil {
		if err := rq.loadUser(ctx, query, nodes, nil,
			func(n *Reservation, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReservationQuery) loadPlace(ctx context.Context, query *PlaceQuery, nodes []*Reservation, init func(*Reservation), assign func(*Reservation, *Place)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Reservation)
	for i := range nodes {
		if nodes[i].place_reservations == nil {
			continue
		}
		fk := *nodes[i].place_reservations
...
func (rq *ReservationQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Reservation, init func(*Reservation), assign func(*Reservation, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Reservation)
	for i := range nodes {
...
func (rq *ReservationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(reservation.Table, reservation.Columns, sqlgraph.NewFieldSpec(reservation.FieldID, field.TypeString))
	_spec.From = rq.sql
