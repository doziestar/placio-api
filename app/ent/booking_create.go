...
func (bc *BookingCreate) check() error {
	if _, ok := bc.mutation.StartDate(); !ok {
		return &ValidationError{Name: "startDate", err: errors.New(`placio_api: missing required field "Booking.startDate"`)}
	}
	if _, ok := bc.mutation.EndDate(); !ok {
		return &ValidationError{Name: "endDate", err: errors.New(`placio_api: missing required field "Booking.endDate"`)}
	}
	if _, ok := bc.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`placio_api: missing required field "Booking.status"`)}
	}
	if _, ok := bc.mutation.BookingDate(); !ok {
		return &ValidationError{Name: "bookingDate", err: errors.New(`placio_api: missing required field "Booking.bookingDate"`)}
	}
	if v, ok := bc.mutation.ID(); ok {
		if err := booking.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`placio_api: validator failed for field "Booking.id": %w`, err)}
		}
	}
	return nil
}
