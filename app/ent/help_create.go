...
func (hc *HelpCreate) check() error {
	if _, ok := hc.mutation.Category(); !ok {
		return &ValidationError{Name: "category", err: errors.New(`placio_api: missing required field "Help.category"`)}
	}
	if _, ok := hc.mutation.Subject(); !ok {
		return &ValidationError{Name: "subject", err: errors.New(`placio_api: missing required field "Help.subject"`)}
	}
	if _, ok := hc.mutation.Body(); !ok {
		return &ValidationError{Name: "body", err: errors.New(`placio_api: missing required field "Help.body"`)}
	}
	if _, ok := hc.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`placio_api: missing required field "Help.status"`)}
	}
	if _, ok := hc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user_id", err: errors.New(`placio_api: missing required field "Help.user_id"`)}
	}
	if v, ok := hc.mutation.UserID(); ok {
		if err := help.UserIDValidator(v); err != nil {
			return &ValidationError{Name: "user_id", err: fmt.Errorf(`placio_api: validator failed for field "Help.user_id": %w`, err)}
		}
	}
	if _, ok := hc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user", err: errors.New(`placio_api: missing required edge "Help.user"`)}
	}
	return nil
}
