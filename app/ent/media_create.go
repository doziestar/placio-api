...
func (mc *MediaCreate) check() error {
	if _, ok := mc.mutation.URL(); !ok {
		return &ValidationError{Name: "URL", err: errors.New(`placio_api: missing required field "Media.URL"`)}
	}
	if _, ok := mc.mutation.MediaType(); !ok {
		return &ValidationError{Name: "MediaType", err: errors.New(`placio_api: missing required field "Media.MediaType"`)}
	}
	if _, ok := mc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "CreatedAt", err: errors.New(`placio_api: missing required field "Media.CreatedAt"`)}
	}
	if _, ok := mc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "UpdatedAt", err: errors.New(`placio_api: missing required field "Media.UpdatedAt"`)}
	}
	if _, ok := mc.mutation.LikeCount(); !ok {
		return &ValidationError{Name: "likeCount", err: errors.New(`placio_api: missing required field "Media.likeCount"`)}
	}
	if _, ok := mc.mutation.DislikeCount(); !ok {
		return &ValidationError{Name: "dislikeCount", err: errors.New(`placio_api: missing required field "Media.dislikeCount"`)}
	}
	if v, ok := mc.mutation.ID(); ok {
		if err := media.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`placio_api: validator failed for field "Media.id": %w`, err)}
		}
	}
	return nil
}
