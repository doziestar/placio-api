...
return fromU, nil
	...
	func (bq *BusinessQuery) QueryStaffs() *StaffQuery {
		query := (&StaffClient{config: bq.config}).Query()
		query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
			if err := bq.prepareQuery(ctx); err != nil {
				return nil, err
			}  
...
func (bq *BusinessQuery) Clone() *BusinessQuery {
	...
	func (bq *BusinessQuery) WithStaffs(opts ...func(*StaffQuery)) *BusinessQuery {
		query := (&StaffClient{config: bq.config}).Query()
		for _, opt := range opts {
			opt(query)
		}
...
func (bq *BusinessQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Business, error) {
	...
	if len(nodes) == 0 {
		...
		if query := bq.withFollowedBusinesses; query != nil {
			if err := bq.loadFollowedBusinesses(ctx, query, nodes, 
				...
				func(n *Business, e *BusinessFollowBusiness){ n.Edges.FollowedBusinesses = append(n.Edges.FollowedBusinesses, e) }); err != nil {
				return nil, err
			}
		}
		if query := bq.withFollowerBusinesses; query != nil {
			if err := bq.loadFollowerBusinesses(ctx, query, nodes, 
				...
				func(n *Business, e *BusinessFollowBusiness){ n.Edges.FollowerBusinesses = append(n.Edges.FollowerBusinesses, e) }); err != nil {
				return nil, err
			}
		}
		...
		if query := bq.withCategoryAssignments; query != nil {
			if err := bq.loadCategoryAssignments(ctx, query, nodes, 
				...
				func(n *Business, e *CategoryAssignment){ n.Edges.CategoryAssignments = append(n.Edges.CategoryAssignments, e) }); err != nil {
				return nil, err
			}
		}
		...
		if query := bq.withBusinessFollowEvents; query != nil {
			if err := bq.loadBusinessFollowEvents(ctx, query, nodes, 
				...
				func(n *Business, e *BusinessFollowEvent){ n.Edges.BusinessFollowEvents = append(n.Edges.BusinessFollowEvents, e) }); err != nil {
				return nil, err
			}
		}
		...
		if query := bq.withStaffs; query != nil {
	...
	func (bq *BusinessQuery) loadStaffs(ctx context.Context, query *StaffQuery, nodes []*Business, init func(*Business), assign func(*Business, *Staff)) error {
			edgeIDs := make([]driver.Value, len(nodes))
			byID := make(map[string]*Business)
			nids := make(map[string]map[*Business]struct{})
			for i, node := range nodes {
			...
			query.Where(func(s *sql.Selector) {
				joinT := sql.Table(business.StaffsTable)
				s.Join(joinT).On(s.C(staff.FieldID), joinT.C(business.StaffsPrimaryKey[1]))
				s.Where(sql.InValues(joinT.C(business.StaffsPrimaryKey[0]), edgeIDs...))
				columns := s.SelectedColumns()
				s.Select(joinT.C(business.StaffsPrimaryKey[0]))
				s.AppendSelect(columns...)
				s.SetDistinct(false)
			})
			if err := query.prepareQuery(ctx); err != nil {
