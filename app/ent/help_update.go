...
func (hu *HelpUpdate) check() error {
	if _, ok := hu.mutation.UserID(); hu.mutation.UserCleared() && !ok {
		return errors.New(`placio_api: clearing a required unique edge "Help.user"`)
	}
	return nil
}
...
func (huo *HelpUpdateOne) sqlSave(ctx context.Context) (_node *Help, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`placio_api: missing "Help.id" for update`)}
	}
