...
func (ac *AmenityCreate) check() error {
	if _, ok := ac.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`placio_api: missing required field "Amenity.name"`)}
	}
	if _, ok := ac.mutation.Icon(); !ok {
		return &ValidationError{Name: "icon", err: errors.New(`placio_api: missing required field "Amenity.icon"`)}
	}
	if v, ok := ac.mutation.ID(); ok {
		if err := amenity.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`placio_api: validator failed for field "Amenity.id": %w`, err)}
		}
	}
	return nil
}
