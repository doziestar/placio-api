...
func (iac *InventoryAttributeCreate) check() error {
	if _, ok := iac.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`ent: missing required field "InventoryAttribute.name"`)}
	}
	if _, ok := iac.mutation.IsMandatory(); !ok {
		return &ValidationError{Name: "is_mandatory", err: errors.New(`ent: missing required field "InventoryAttribute.is_mandatory"`)}
	}
	if v, ok := iac.mutation.DataType(); ok {
		if err := inventoryattribute.DataTypeValidator(v); err != nil {
			return &ValidationError{Name: "data_type", err: fmt.Errorf(`ent: validator failed for field "InventoryAttribute.data_type": %w`, err)}
		}
	}
	if v, ok := iac.mutation.ID(); ok {
		if err := inventoryattribute.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`ent: validator failed for field "InventoryAttribute.id": %w`, err)}
		}
	}
	return nil
}
