...
func (bc *BusinessCreate) check() error {
	if _, ok := bc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`placio_api: missing required field "Business.name"`)}
	}
	if _, ok := bc.mutation.FollowerCount(); !ok {
		return &ValidationError{Name: "follower_count", err: errors.New(`placio_api: missing required field "Business.follower_count"`)}
	}
	if _, ok := bc.mutation.FollowingCount(); !ok {
		return &ValidationError{Name: "following_count", err: errors.New(`placio_api: missing required field "Business.following_count"`)}
	}
	if v, ok := bc.mutation.ID(); ok {
		if err := business.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`placio_api: validator failed for field "Business.id": %w`, err)}
		}
	}
	return nil
}
