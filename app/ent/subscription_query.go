...
func (sq *SubscriptionQuery) QueryPrice() *PriceQuery {
...
func (sq *SubscriptionQuery) WithPrice(opts ...func(*PriceQuery)) *SubscriptionQuery {
	query := (&PriceClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(subscription.FieldStartDate).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Subscription.Query().
//		Select(subscription.FieldStartDate).
//		Scan(ctx, &v)
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if sq.withUser != nil || sq.withPlan != nil || sq.withPrice != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withPrice; query != nil {
		if err := sq.loadPrice(ctx, query, nodes, nil,
			func(n *Subscription, e *Price) { n.Edges.Price = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubscriptionQuery) loadPrice(ctx context.Context, query *PriceQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Price)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Subscription)
	for i := range nodes {
		if nodes[i].price_subscriptions == nil {
			continue
		}
		fk := *nodes[i].price_subscriptions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "price_subscriptions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
