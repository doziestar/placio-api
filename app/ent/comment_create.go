...
func (cc *CommentCreate) check() error {
	if _, ok := cc.mutation.Content(); !ok {
		return &ValidationError{Name: "Content", err: errors.New(`placio_api: missing required field "Comment.Content"`)}
	}
	...
	if _, ok := cc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "CreatedAt", err: errors.New(`placio_api: missing required field "Comment.CreatedAt"`)}
	}
	if _, ok := cc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "UpdatedAt", err: errors.New(`placio_api: missing required field "Comment.UpdatedAt"`)}
	}
	if v, ok := cc.mutation.ID(); ok {
		if err := comment.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`placio_api: validator failed for field "Comment.id": %w`, err)}
		}
	}
	return nil
}
