...
func (frc *FeatureReleaseCreate) check() error {
	if _, ok := frc.mutation.FeatureName(); !ok {
		return &ValidationError{Name: "feature_name", err: errors.New(`placio_api: missing required field "FeatureRelease.feature_name"`)}
	}
	if _, ok := frc.mutation.State(); !ok {
		return &ValidationError{Name: "state", err: errors.New(`placio_api: missing required field "FeatureRelease.state"`)}
	}
	...
	if _, ok := frc.mutation.ReleaseDate(); !ok {
		return &ValidationError{Name: "release_date", err: errors.New(`placio_api: missing required field "FeatureRelease.release_date"`)}
	}
	if v, ok := frc.mutation.ID(); ok {
		if err := featurerelease.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`placio_api: validator failed for field "FeatureRelease.id": %w`, err)}
		}
	}
	return nil
}
