...
func (tq *TemplateQuery) Order(o ...template.OrderOption) *TemplateQuery {
	...
	func (tq *TemplateQuery) QueryWebsites() *WebsiteQuery {
		query := (&WebsiteClient{config: tq.config}).Query()
...
func (tq *TemplateQuery) Clone() *TemplateQuery {
	...
	return &TemplateQuery{
		...
		predicates: append([]predicate.Template{}, tq.predicates...),
			withWebsites: tq.withWebsites.Clone(),
	...
	func (tq *TemplateQuery) WithWebsites(opts ...func(*WebsiteQuery)) *TemplateQuery {
		query := (&WebsiteClient{config: tq.config}).Query()
		for _, opt := range opts {
			opt(query)
		}
		tq.withWebsites = query
		return tq
	}
...
func (tq *TemplateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Template, error) {
	...
	if len(nodes) == 0 {
		...
		if query := tq.withWebsites; query != nil {
			if err := tq.loadWebsites(ctx, query, nodes, 
				func(n *Template){ n.Edges.Websites = []*Website{} },
				func(n *Template, e *Website){ n.Edges.Websites = append(n.Edges.Websites, e) }); err != nil {
				return nil, err
			}
		}
	return nodes, nil
	...
	func (tq *TemplateQuery) loadWebsites(ctx context.Context, query *WebsiteQuery, nodes []*Template, init func(*Template), assign func(*Template, *Website)) error {
			...
			query.Where(predicate.Website(func(s *sql.Selector) {
				s.Where(sql.InValues(s.C(template.WebsitesColumn), fks...))
			}))
			...
			for _, n := range neighbors {
				fk := n.template_websites
					if fk == nil {
						return fmt.Errorf(`foreign-key "template_websites" is nil for node %v`, n.ID)
					}
				node, ok := nodeids[*fk]
				if !ok {
					return fmt.Errorf(`unexpected referenced foreign-key "template_websites" returned %v for node %v`, *fk, n.ID)
				}
				assign(node, n)
			}
		return nil
