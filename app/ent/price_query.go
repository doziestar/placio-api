...
func (pq *PriceQuery) QueryPlan() *PlanQuery {
...
func (pq *PriceQuery) QuerySubscriptions() *SubscriptionQuery {
...
func (pq *PriceQuery) WithPlan(opts ...func(*PlanQuery)) *PriceQuery {
	query := (&PlanClient{config: pq.config}).Query()
...
func (pq *PriceQuery) WithSubscriptions(opts ...func(*SubscriptionQuery)) *PriceQuery {
	query := (&SubscriptionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PriceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Price, error) {
	...
	if query := pq.withPlan; query != nil {
		if err := pq.loadPlan(ctx, query, nodes, nil,
			func(n *Price, e *Plan) { n.Edges.Plan = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withSubscriptions; query != nil {
		if err := pq.loadSubscriptions(ctx, query, nodes,
			func(n *Price) { n.Edges.Subscriptions = []*Subscription{} },
			func(n *Price, e *Subscription) { n.Edges.Subscriptions = append(n.Edges.Subscriptions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PriceQuery) loadPlan(ctx context.Context, query *PlanQuery, nodes []*Price, init func(*Price), assign func(*Price, *Plan)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Price)
	for i := range nodes {
		if nodes[i].plan_prices == nil {
			continue
		}
		fk := *nodes[i].plan_prices
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "plan_prices" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PriceQuery) loadSubscriptions(ctx context.Context, query *SubscriptionQuery, nodes []*Price, init func(*Price), assign func(*Price, *Subscription)) error {
	...
	query.Where(predicate.Subscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(price.SubscriptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.price_subscriptions
		if fk == nil {
			return fmt.Errorf(`foreign-key "price_subscriptions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "price_subscriptions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
