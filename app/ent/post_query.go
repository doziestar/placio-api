...
func (pq *PostQuery) QueryBusinessAccount() *BusinessQuery {
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withBusinessAccount: pq.withBusinessAccount.Clone(),
...
func (pq *PostQuery) WithBusinessAccount(opts ...func(*BusinessQuery)) *PostQuery {
	query := (&BusinessClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withBusinessAccount = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withBusinessAccount; query != nil {
		if err := pq.loadBusinessAccount(ctx, query, nodes, nil,
			func(n *Post, e *Business) { n.Edges.BusinessAccount = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *PostQuery) loadBusinessAccount(ctx context.Context, query *BusinessQuery, nodes []*Post, init func(*Post), assign func(*Post, *Business)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "business_posts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PostQuery) loadMedias(ctx context.Context, query *MediaQuery, nodes []*Post, init func(*Post), assign func(*Post, *Media)) error {
	...
	for _, n := range neighbors {
		fk := n.post_medias
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_medias" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_medias" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
