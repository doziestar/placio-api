...
func (bfuc *BusinessFollowUserCreate) check() error {
	if _, ok := bfuc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "CreatedAt", err: errors.New(`placio_api: missing required field "BusinessFollowUser.CreatedAt"`)}
	}
	if _, ok := bfuc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "UpdatedAt", err: errors.New(`placio_api: missing required field "BusinessFollowUser.UpdatedAt"`)}
	}
	return nil
}
