...
func (iq *InvitationQuery) QueryBusinessAccount() *BusinessAccountQuery {
	query := (&BusinessAccountClient{config: iq.config}).Query()
...
func (iq *InvitationQuery) Clone() *InvitationQuery {
	...
	return &InvitationQuery{
		...
		withBusinessAccount: iq.withBusinessAccount.Clone(),
...
func (iq *InvitationQuery) WithBusinessAccount(opts ...func(*BusinessAccountQuery)) *InvitationQuery {
	query := (&BusinessAccountClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withBusinessAccount = query
	return iq
}
...
func (iq *InvitationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invitation, error) {
	...
	if query := iq.withBusinessAccount; query != nil {
		if err := iq.loadBusinessAccount(ctx, query, nodes,
			func(n *Invitation) { n.Edges.BusinessAccount = []*BusinessAccount{} },
			func(n *Invitation, e *BusinessAccount) { n.Edges.BusinessAccount = append(n.Edges.BusinessAccount, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *InvitationQuery) loadBusinessAccount(ctx context.Context, query *BusinessAccountQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *BusinessAccount)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Invitation)
	nids := make(map[int]map[*Invitation]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(invitation.BusinessAccountTable)
		s.Join(joinT).On(s.C(businessaccount.FieldID), joinT.C(invitation.BusinessAccountPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(invitation.BusinessAccountPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(invitation.BusinessAccountPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Invitation]struct{}{byID[outValue]: {}}
	...
	neighbors, err := withInterceptors[[]*BusinessAccount](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
