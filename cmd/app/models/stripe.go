...
import (
	...
	"github.com/stripe/stripe-go/cust"
	"github.com/stripe/stripe-go/inv"
)
...
func RetrieveSubscription(id string) (*stripe.Subscription, error) {
	subscription, err := sub.Get(id, &stripe.SubscriptionParams{
...
func UpdateCustomer(id string, token string) (*stripe.Customer, error) {
	customer, err := cust.Update(id, &stripe.CustomerParams{
		Source: &stripe.SourceParams{
			Token: &token,
		},
	})
